...
var renderer = new THREE.WebGLRenderer({ antialias: true, canvas: document.getElementById("canvas") });
...
function onMouseMove(event) {
    ...
    if (intersects.length > 0) {
        for (var i = 0; i < intersects.length; i++) {
            ...
            this.tl.to(intersects[i].object.scale, 1.5, { x: 1.5, y: 1.5, z: 1.5, ease: Expo.easeOut })
            this.tl.to(intersects[i].object.scale, 1, { x: 10, y: 10, z: 10, ease: Expo.easeOut })
            // this.tl.to(intersects[i].object.position, .5, {x: 20, ease: Expo.easeOut})
